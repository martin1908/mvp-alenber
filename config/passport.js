...
passport.deserializeUser((id, done) => {
  request.get({url:'https://alenber.herokuapp.com/api/id-passport/'+ id.usuario._id }, (err, response, user) => {
...
passport.use(new LocalStrategy({ usernameField: 'email', passwordField: 'password' }, (email, password, done) => {
  ...
  request.post({url:'https://alenber.herokuapp.com/api/login', form: credenciales }, (err, response, user) => {
    if (err) { return done(err) }
    var data = JSON.parse(user)
    if (!data) {
      return done(null, false, {message: 'Incorrect email asdasd.'})
    }
